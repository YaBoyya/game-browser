...
function FavouritesButton({cookie, id, setErrorMessage}) {
  ...
  useEffect(() => {getAdded()}, []);
  if(added) {
    return <button onClick={remove} className="text-center h-fit w-fit p-3 bg-blue-400 mb-4">Remove from favourites</button>
  } else {
    return <button onClick={add} className="text-center h-fit w-fit p-3 bg-blue-400 mb-4">Add to favourites</button>
  }
}
